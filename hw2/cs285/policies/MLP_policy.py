...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        fool_pytorch = -log_pi * advantages
        loss = torch.sum(fool_pytorch)
        ...
        if self.nn_baseline:
            ...
            q_values_normalize = ptu.from_numpy((q_values - np.mean(q_values)) / np.std(q_values))
            pred_baseline = self.baseline(observations)
            b_loss = self.baseline_loss(pred_baseline, q_values_normalize)
